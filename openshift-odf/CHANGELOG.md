...
- `scripts/post_uninstall_audit.sh`: accepts a kept `openshift-storage` namespace when non-ODF operators remain (then sweeps it for ODF residue objects), treats retained LSO CRDs as OK, audits the two new CRD groups, `ceph-csi` SCCs, the ODF mutating webhook, and consoleplugins.
