...
- Do not edit the Rook `CephCluster`, `CephBlockPool`, `CephFilesystem`, or `CephObjectStore` CRs directly on an ODF cluster. `ocs-operator` owns and reconciles them from the `StorageCluster` CR; hand edits are reverted and can break reconciliation. Change behavior through `StorageCluster` (and documented overrides) instead.
  - **Exception (ODF 4.20 and 4.22 SNO):** both releases have known regressions on single-OSD SNO where pool replica sizes are not reduced. The permitted scope of direct Rook CR editing is: freeze `cephBlockPools`, `cephObjectStores`, **and** `cephFilesystems` (`managedResources.*.reconcileStrategy: ignore` — omitting `cephFilesystems` lets ocs-operator revert the `CephFilesystem` patches), then patch the frozen CRs for pool sizing, the empty `topologyKey` (`""` + `DoNotSchedule`) on the MDS/RGW placements, and the CSI controller replica count via the `Driver` CRs (`drivers.csi.ceph.io`, not `OperatorConfig`). Do not extend direct editing beyond that scope. The remaining steps are version-specific and are **not** reproduced here — follow the per-version procedure in `references/validated-odf-sno.md`, and use `scripts/render_sno_remediation.py --release {4.20,4.22}` to render the deterministic patches for review. Every part of this is temporary: restore `reconcileStrategy: manage` after upgrading to a release with the fix. The freeze also blocks **uninstall**: while `reconcileStrategy: ignore` is set, graceful teardown stalls because `ocs-operator` skips the ignored resources and rook will not delete the `CephCluster` while their CRs remain, so those CRs must be deleted directly. Observed on 4.22.1 for the `CephBlockPool` CRs; expect the same for a frozen `CephFilesystem`/`CephObjectStore` (see `references/maintenance-uninstall.md`).
