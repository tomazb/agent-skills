...
- **Not enough vCPUs, workloads Pending on `Insufficient cpu`, or a lab SNO that should run ODF more modestly**: run the CPU Request Budget check in `references/install-and-preflight.md`, then the CPU-request floor in `references/validated-odf-sno.md` (`render_sno_remediation.py --lab-resources`). Lab/non-production only; never `resourceProfile: lean` on 4.20/4.22 SNO.
...
- Do not edit the Rook `CephCluster`, `CephBlockPool`, `CephFilesystem`, or `CephObjectStore` CRs directly on an ODF cluster. `ocs-operator` owns and reconciles them from the `StorageCluster` CR; hand edits are reverted and can break reconciliation. Change behavior through `StorageCluster` (and documented overrides) instead.
  - **Exception (ODF 4.20 and 4.22 SNO):** both releases have known regressions on single-OSD SNO where pool replica sizes are not reduced. Permitted scope of direct Rook CR editing: freeze `cephBlockPools`, `cephObjectStores`, **and** `cephFilesystems` via `managedResources.*.reconcileStrategy: ignore`, then patch those frozen CRs for pool sizing, MDS/RGW placement, MDS/RGW resource requests (the CPU-request floor), and CSI controller replicas. Do not extend direct editing beyond that scope, and restore `reconcileStrategy: manage` after upgrading to a release with the fix. The freeze also stalls teardown, so uninstall needs its own procedure. Per-version steps are in `references/validated-odf-sno.md` (rendered for review by `scripts/render_sno_remediation.py --release {4.20,4.22}`); read `references/maintenance-uninstall.md` before uninstalling.
