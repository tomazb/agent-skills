...
- Do not edit the Rook `CephCluster`, `CephBlockPool`, `CephFilesystem`, or `CephObjectStore` CRs directly on an ODF cluster. `ocs-operator` owns and reconciles them from the `StorageCluster` CR; hand edits are reverted and can break reconciliation. Change behavior through `StorageCluster` (and documented overrides) instead.
  - **Exception (ODF 4.20 and 4.22 SNO):** Both ODF 4.20 and 4.22 have known SNO regressions where pool replica sizes are not reduced for single-OSD deployments. After setting the relevant `managedResources.*.reconcileStrategy: ignore` fields in the `StorageCluster`, it is necessary to patch the affected Rook CRs directly and set pool sizes via `ceph osd pool set`. On **both 4.20 and 4.22.1 SNO** (CephFS validated on each), freeze `cephBlockPools`, `cephObjectStores`, **and** `cephFilesystems` (otherwise ocs-operator reverts the `CephFilesystem` patches), then: (1) size the `CephBlockPool`, `CephObjectStore`, and `CephFilesystem` metadata/data pools to `1`; (2) fix the empty `topologyKey` (`""` + `DoNotSchedule`) that ocs-operator sets on the `CephFilesystem` MDS and `CephObjectStore` RGW placements by patching it to a valid key + `ScheduleAnyway`; and (3) reduce CSI controller replicas by patching the `Driver` CRs (`drivers.csi.ceph.io`) to `replicas: 1` rather than through `OperatorConfig` (which is reverted). Version-specific extras: on **4.20** change the `CephBlockPool` `failureDomain` from `osd` to `host` and remove `replicasPerFailureDomain` (Rook rejects `size=1` + `failureDomain: osd` + `replicasPerFailureDomain: 1`); on **4.22.1** remove `replicasPerFailureDomain` from the `CephObjectStore`/`CephFilesystem` metadata/data pools (Ceph 20.2 "tentacle" rejects `size=1` + `replicasPerFailureDomain: 1` there) and set minimal per-component `spec.resources` requests to avoid SNO CPU-request starvation (do not use `resourceProfile: lean` — it traps the StorageCluster in `Progressing`). Document these as temporary workarounds and re-enable `reconcileStrategy: manage` when ODF is upgraded to a version with the fix. See `references/validated-odf-sno.md` for the complete per-version procedure; `scripts/render_sno_remediation.py` renders the deterministic 4.20/4.22 patches for review.
...
- ODF on SNO uses a single replica-1 device set with `replica: 1` (`resiliencePolicy` reduced) and reduced mon/mgr counts. Discover the actual counts rather than assuming them — **ODF 4.20 does not reduce the mon count on SNO** and runs 3 mons + 1 mgr, all on the single node (see `references/validated-odf-sno.md`); do not treat that as a misconfiguration to "fix". A compact three-node cluster remains a multi-node deployment and uses the standard three OSDs across three failure domains. Do not copy single-node settings into a compact or other multi-node production plan without explicit direction.
...
- Cluster topology: SNO, compact (3-node), or multi-node; OpenShift/OKD version; node roles; target MachineConfigPool; and failure domain layout (host, rack, zone, or datacenter). Collect `oc get infrastructure cluster -o jsonpath='{.status.controlPlaneTopology}'` — a result of `SingleReplica` confirms SNO and triggers the SNO install path in `references/install-and-preflight.md`.
...
- Drive changes through the `StorageCluster` CR and OLM Subscription; never hand-edit the ODF-owned Rook CRs except for the version-scoped ODF 4.20 and 4.22 SNO pool, `topologyKey`, and resource workarounds in Core Safety Rules and `references/validated-odf-sno.md`.
