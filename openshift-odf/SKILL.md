...
- Do not edit the Rook `CephCluster`, `CephBlockPool`, `CephFilesystem`, or `CephObjectStore` CRs directly on an ODF cluster. `ocs-operator` owns and reconciles them from the `StorageCluster` CR; hand edits are reverted and can break reconciliation. Change behavior through `StorageCluster` (and documented overrides) instead.
  - **Exception (ODF 4.20 and 4.22 SNO):** Both ODF 4.20 and 4.22 have known SNO regressions where pool replica sizes are not reduced for single-OSD deployments. After setting the relevant `managedResources.*.reconcileStrategy: ignore` fields in the `StorageCluster`, it is necessary to patch the affected Rook CRs directly and set pool sizes via `ceph osd pool set`. On **ODF 4.22 SNO**, freeze `cephBlockPools` and `cephObjectStores` and patch only the `CephBlockPool` and `CephObjectStore` CRs; do not enable CephFS — its pool-reconciliation path was not validated for 4.22 SNO. On **ODF 4.20 SNO**, CephFS was validated, so additionally freeze `cephFilesystems` (otherwise ocs-operator reverts the `CephFilesystem` patches) and patch the `CephFilesystem` CR, plus these 4.20-only steps not needed on 4.22: (1) change the `CephBlockPool` `failureDomain` from `osd` to `host` and remove `replicasPerFailureDomain` (Rook rejects `size=1` + `failureDomain: osd` + `replicasPerFailureDomain: 1`); (2) size the `CephFilesystem` and `CephObjectStore` metadata/data pools to `1` the same way; (3) fix the empty `topologyKey` (`""` + `DoNotSchedule`) that ocs-operator sets on the `CephFilesystem` MDS and `CephObjectStore` RGW placements by patching it to a valid key + `ScheduleAnyway`; and (4) reduce CSI controller replicas by patching the `Driver` CRs (`drivers.csi.ceph.io`) to `replicas: 1` rather than through `OperatorConfig` (insufficient in 4.20). Document these as temporary workarounds and re-enable `reconcileStrategy: manage` when ODF is upgraded to a version with the fix. See `references/validated-odf-sno.md` for the complete per-version procedure; `scripts/render_sno_remediation.py` renders the deterministic 4.20 patches for review.
...
- Drive changes through the `StorageCluster` CR and OLM Subscription; never hand-edit the ODF-owned Rook CRs except for the version-scoped ODF 4.20 and 4.22 SNO pool workarounds in Core Safety Rules and `references/validated-odf-sno.md`.
