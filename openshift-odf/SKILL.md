...
- Do not edit the Rook `CephCluster`, `CephBlockPool`, `CephFilesystem`, or `CephObjectStore` CRs directly on an ODF cluster. `ocs-operator` owns and reconciles them from the `StorageCluster` CR; hand edits are reverted and can break reconciliation. Change behavior through `StorageCluster` (and documented overrides) instead.
  - **Exception (ODF 4.20 and 4.22 SNO):** Both ODF 4.20 and 4.22 have known SNO regressions where pool replica sizes are not reduced for single-OSD deployments. After setting `managedResources.cephBlockPools.reconcileStrategy: ignore` and `managedResources.cephObjectStores.reconcileStrategy: ignore` in the `StorageCluster`, it is necessary to patch the `CephBlockPool` and `CephObjectStore` CRs directly and set pool sizes via `ceph osd pool set`. Do not enable CephFS with this workaround: its pool-reconciliation path was not validated for ODF 4.20 or 4.22 SNO. Document this as a temporary workaround and re-enable `reconcileStrategy: manage` when ODF is upgraded to a version with the fix. ODF 4.20 has two additional steps not present in 4.22: the `CephBlockPool` must also have `failureDomain` changed from `osd` to `host` with `replicasPerFailureDomain` removed (Rook validation rejects `size=1` + `failureDomain: osd` + `replicasPerFailureDomain: 1`), and CSI controller replicas must be fixed by patching `Driver` CRs (`drivers.csi.ceph.io`) to `replicas: 1` rather than through `OperatorConfig` (which is insufficient in 4.20). See `references/validated-odf-sno.md` for the complete per-version procedure.
