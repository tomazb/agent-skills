...
def test_frontmatter_name_mismatch(validator, package_factory):
    root = package_factory(skill_text="---\nname: wrong-name\ndescription: Use when demo.\n---\n\n# Routing\n\n## Routing\n\nr\n\n## Core Safety Rules\n\ns\n\n## Required Source Checks\n\nc\n\n## Inputs To Collect\n\ni\n\n## Output Expectations\n\no\n")
    issues = validator.check_frontmatter(root)
    assert any("wrong-name" in issue for issue in issues)


def test_frontmatter_description_missing(validator, package_factory):
    root = package_factory(skill_text="---\nname: openshift-lvm-storage\n---\n\n# Routing\n\n## Routing\n\nr\n\n## Core Safety Rules\n\ns\n\n## Required Source Checks\n\nc\n\n## Inputs To Collect\n\ni\n\n## Output Expectations\n\no\n")
    issues = validator.check_frontmatter(root)
    assert any("missing frontmatter description" in issue for issue in issues)


def test_frontmatter_description_must_start_with_use_when(validator, package_factory):
