...
def test_version_sync_mismatch(validator, package_factory):
    root = package_factory(
        package_json_text='{"name":"how-to-speak-winston-framework","version":"9.9.9"}\n',
    )
...
def test_version_sync_invalid_json(validator, package_factory):
    root = package_factory(
        package_json_text='{"name": "demo", "version": "1.2.3"'
    )
    issues = validator.check_version_sync(root)
...
def test_changelog_missing_version_file_is_noop(validator, package_factory):
    root = package_factory(include_version=False, include_changelog=True)
    assert validator.check_changelog_version(root) == []


def test_changelog_missing_changelog(validator, package_factory):
