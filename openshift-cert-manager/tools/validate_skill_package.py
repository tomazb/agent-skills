...
EXPECTED_NAME = "openshift-cert-manager"
...
REQUIRED_FILES = [
    ...
    "CHANGELOG.md",
    "package.json",
    "scripts/discover_tls.py",
    "scripts/check_http01_reachability.py",
...
REQUIRED_HELPER_INVOCATIONS = [
    ...
    (
        "references/acme-http01-proof.md",
        "python3 scripts/check_http01_reachability.py",
        "HTTP-01 reachability helper invocation",
    ),
    (
        "references/validation-troubleshooting.md",
        "python3 scripts/discover_tls.py --context \"<oc-context>\" --execute",
...
def validate_root(root: Path) -> list[str]:
    ...
    issues.extend(check_phrase_group(all_text, SAFETY_PHRASES, "platform TLS safety"))
    issues.extend(check_phrase_group(all_text, CERT_MANAGER_PHRASES, "cert-manager"))
    issues.extend(check_phrase_group(all_text, HTTP01_PHRASES, "HTTP-01"))
    issues.extend(check_phrase_group(all_text, DNS01_PHRASES, "DNS-01"))
    issues.extend(check_phrase_group(all_text, SNO_PHRASES, "SNO"))
    issues.extend(check_phrase_group(all_text, OPENSHIFT_PHRASES, "OpenShift OLM"))
