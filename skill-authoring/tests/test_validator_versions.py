...
def test_bump_version_fails_when_readme_version_line_missing(
    ...
    (root / "package.json").write_text(
        json.dumps({"name": "skill-authoring", "version": "0.1.0"}, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    assert json.loads((root / "package.json").read_text(encoding="utf-8"))["version"] == "0.1.0"
...
def test_bump_version_rolls_back_when_a_file_update_fails(
    ...
    assert json.loads(package_file.read_text(encoding="utf-8"))["version"] == "0.1.0"
    assert version_file.read_text(encoding="utf-8") == "0.1.0\n"
    assert "Current version: **0.1.0**" in readme_file.read_text(encoding="utf-8")
...
def test_version_sync_mismatch(validator, package_factory):
    root = package_factory(
        package_json_text='{"name":"skill-authoring","version":"9.9.9"}\n'
    )
...
def test_version_sync_invalid_package_json(validator, package_factory):
    root = package_factory(
        package_json_text='{"name": "skill-authoring", "version": "0.1.0"'
    )
    issues = validator.check_version_sync(root)
...
def test_version_sync_rejects_invalid_semver_even_when_versions_match(
    ...
    root = package_factory(
        package_json_text='{"name":"skill-authoring","version":"01.2.3"}\n'
    )
...
def test_changelog_version_mismatch(validator, package_factory):
    ...
    assert len(issues) == 1
    assert "does not contain a heading for VERSION '0.1.0'" in issues[0]
...
def test_readme_version_mismatch(validator, package_factory):
    root = package_factory(readme_text="# Skill Authoring\n\nCurrent version: **9.9.9**\n")
    issues = validator.check_readme_version(root)
    assert issues == ["README.md version does not match VERSION '0.1.0'."]
