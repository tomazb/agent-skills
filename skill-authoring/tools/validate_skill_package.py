...
def check_skill_frontmatter(skill_text: str) -> list[str]:
    ...
    if fields.get("name") != "skill-authoring":
        issues.append("SKILL.md: frontmatter name must be 'skill-authoring'.")
...
def check_guidance_guards(skill_text: str) -> list[str]:
    issues: list[str] = []
    body = "\n".join(visible_lines(skill_text))

    if "validate_skill_collection.py" not in body:
        issues.append("SKILL.md: validation guidance must name validate_skill_collection.py.")
    if "run_test_suite.py" not in body:
        issues.append("SKILL.md: validation guidance must name run_test_suite.py.")
    if "package.json" not in body:
        issues.append("SKILL.md: versioning guidance must cover package.json sync.")
    if "AGENTS.md" not in body:
        issues.append("SKILL.md: index guidance must cover the AGENTS.md inventory.")
