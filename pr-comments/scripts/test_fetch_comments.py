"""Tests for comment resolution and auth-precheck behavior (no network calls).
...
import fetch_github_review_comments as comments_module
...
class TestAuthentication(unittest.TestCase):
    def test_main_checks_auth_before_pr_lookup(self):
        calls = []

        def fake_run_command(args, error_msg="Command failed"):
            calls.append((args, error_msg))
        ...
        with patch.object(comments_module, "run_command", side_effect=fake_run_command):
            with self.assertRaises(SystemExit):
                comments_module.main()

        assert calls[0][0] == ["gh", "auth", "status"]

    def test_ensure_gh_authenticated_exits_with_clear_error(self):
        stderr = StringIO()
        result = Mock(returncode=1, stdout="", stderr="not logged in\n")

        with patch("fetch_github_review_comments.subprocess.run", return_value=result):
            with patch("sys.stderr", stderr):
                with self.assertRaises(SystemExit):
                    comments_module.ensure_gh_authenticated()

        assert "gh auth status failed" in stderr.getvalue()
        assert "gh auth login" in stderr.getvalue()
