...
def update_readme_version(readme_path: Path, version: str) -> None:
    if not readme_path.exists():
        return
    text = readme_path.read_text(encoding="utf-8")
    updated = README_VERSION_RE.sub(rf"\g<1>{version}\g<2>", text)
...
def main(argv: list[str]) -> int:
    ...
    update_readme_version(readme_file, version)
