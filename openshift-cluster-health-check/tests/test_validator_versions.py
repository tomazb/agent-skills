...
def test_version_sync_mismatch(validator, package_factory):
    root = package_factory(
        package_json_text='{"name":"openshift-cluster-health-check","version":"9.9.9"}\n',
    )
...
def test_version_sync_missing_version_file_is_noop(validator, package_factory):
    root = package_factory(include_version=False, include_package_json=True)
    assert validator.check_version_sync(root) == []


def test_version_sync_missing_package_json_is_noop(validator, package_factory):
...
def test_version_sync_invalid_package_json(validator, package_factory):
    root = package_factory(
        package_json_text='{"name": "openshift-cluster-health-check", "version": "1.2.3"'
    )
    issues = validator.check_version_sync(root)
...
def test_changelog_version_missing_changelog_is_noop(validator, package_factory):
